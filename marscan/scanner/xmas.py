...
class XmasScan(BaseScan):
    """
    Performs a TCP XMAS scan.

    This scan type sends a TCP packet with the FIN, PSH, and URG flags set.
    ...
    def _scan_single_port(self, port: int) -> bool:
        """
        Scans a single port using the TCP XMAS scan technique.
        ...
        try:
            ...
            tcp_layer = TCP(dport=port, sport=random.randint(1024, 65535), flags="FPU")
            if self.tcp_window:
            ...
            self.logger.debug(f"[*] Port {port}: Exception during XMAS scan: {e}")
            return False

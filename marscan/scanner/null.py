...
class NullScan(BaseScan):
    ...
    This scan type sends a TCP packet with no flags set.
    ...
    def _scan_single_port(self, port: int) -> bool:
        """
        Scans a single port using the TCP NULL scan technique.
        ...
        try:
            ...
            tcp_layer = TCP(dport=port, sport=random.randint(1024, 65535), flags="")
            if self.tcp_window:
            ...
            self.logger.debug(f"[*] Port {port}: Exception during NULL scan: {e}")
            return False
